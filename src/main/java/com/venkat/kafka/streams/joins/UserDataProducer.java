...
public class UserDataProducer {
    ...
    public void produce() throws InterruptedException, ExecutionException {
        ...
        producer.send(purchaseRecord("venkatram", "Computer (4)")).get();
        producer.send(userRecord("venkatram", "First=venkatram,Last=veerareddy,GitHub=venkatram64")).get();
        producer.send(purchaseRecord("venkatram", "Books (4)")).get();
        producer.send(userRecord("venkatram", null)).get(); // delete for cleanup

        Thread.sleep(10000);

        // 5 - we create a user, but it gets deleted before any purchase comes through
        System.out.println("\nExample 5 - user then delete then data\n");
        producer.send(userRecord("ani", "First=ani")).get();
        producer.send(userRecord("ani", null)).get(); // that's the delete record
        producer.send(purchaseRecord("ani", "Apache Kafka store (5)")).get();
